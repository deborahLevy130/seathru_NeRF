        ...
        if jax.host_id() == 0:
            ...
            if step == init_step or step % config.print_every == 0:
                ...
                # for k, v in stats_split.items():
                    ...
                    # wandb.log({f'train/{k}': wandb.Histogram(v)}, commit=False)
                ...
                #     summ_fn_wandb(f'train/avg_{k}', v, step)
                # for k, v in max_stats.items():
                #     # summ_fn(f'train_max_{k}', v)
                #     summ_fn_wandb(f'train/max_{k}', v, step)
            ...
            if jax.host_id() == 0:
                ...
                for k, v in vis_suite.items():
                    ...
                    if k in ['sigma_bs','sigma_atten']:
                        ...
                        # utils.save_img_f32(np.asarray(v), os.path.join(config.checkpoint_dir, f'{k}_{step}.tiff'))
