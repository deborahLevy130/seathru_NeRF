...
def uw_acc_weights_loss(rays, batch, rendering, config, bs_mult):
...
def uw_acc_tran_loss(ray_history, renderings, batch, config, sig_mult):
...
def create_train_step(model: models.Model,
    ...
    def train_step(
        ...
        def loss_fn(variables):
            ...
            if config.use_uw_acc_weights_loss:
                losses['uw_w_acc'] = uw_acc_weights_loss(rays, batch, renderings, config, bs_mult)

            if config.use_uw_acc_trans_loss:
                losses['uw_t_acc'] = uw_acc_tran_loss(ray_history, renderings, batch, config, sig_mult)
