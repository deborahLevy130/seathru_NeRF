...
from internal.models import UWMLP
...
def compute_data_loss(batch, renderings, rays, config): #TODO: ensure that in the rendering we get also the density and th Bs image
...
def uw_bs_initial_loss(rays, batch, rendering, config, bs_mult): #TODO: chech multiscale loss option
  last_ray_results = rendering[-1]
  weights = last_ray_results['weights'].sum(axis = -1)
  # weights = last_ray_results['density']
  # weights = last_ray_results['trans']
  data_loss = 0.
  # for ray_results in rendering:
  #   weights = ray_results['density']
  # # # rendering contain results from all MLPs, last is the UWMLP
  #   data_loss  += jnp.mean((-jnp.log(jnp.exp(-jnp.abs(1-weights.sum(axis=-1))/0.1)+jnp.exp(-jnp.abs(weights.sum(axis=-1))/0.1))))

  # data_loss += jnp.mean((-jnp.log(jnp.exp(-jnp.abs(1 - weights) / 0.1) + jnp.exp(-jnp.abs(weights) / 0.1))))
  # data_loss += jnp.mean((
  # #     (-jnp.log(jnp.exp(-jnp.abs(1 - weights) / 0.1) +jnp.exp(-jnp.abs(weights) / 0.1)))))
  # data_loss += jnp.mean((
  #   (1/(1+jnp.exp(-10*weights+5)) )*(1-(1/(1+jnp.exp(-10*weights+1000))))))

    # data_loss+=jnp.mean((1/(1+jnp.exp(-10*weights+5)))*(1-(1/(1+jnp.exp(-10*weights+50)))))
  data_loss += jnp.mean((
        (-jnp.log(jnp.exp(-jnp.abs(1 - weights) / 0.1) +1*jnp.exp(-jnp.abs(weights) / 0.1)))))
  # data_loss+= jnp.mean(-jnp.log(jnp.exp(-jnp.abs(1 - weights.sum(axis=-1)) / 0.1))*jax.lax.stop_gradient(weights.sum(axis=-1)))
...
def uw_sig_obj_loss(ray_history, renderings,batch, config, sig_mult):
  last_ray_results = renderings[-1]
  weights = last_ray_results['trans']
  # weights_bs = last_ray_results['bs_weights']
  # weights_direct = last_ray_results['weights'].sum(axis=-1)

  data_loss = 0
  # last_ray_results = renderings[-1]
  #
  # # weights = last_ray_results['joint_weights']
  # for ray_results in ray_history:
  #   density = ray_results['density']
  # # data_loss  =  jnp.mean((-jnp.log(jnp.exp(-jnp.abs(1.0-weights)/0.1))))
  # # data_loss  =  jnp.mean((-jnp.log(jnp.exp(-jnp.abs(1.0-weights.sum(axis=-2))/0.01)+jnp.exp(-jnp.abs(weights.sum(axis=-2))/0.01))))
  # # data_loss  =  jnp.mean((-jnp.log(jnp.exp(-jnp.abs(1.0-(weights.sum(axis=-2)))/0.1))))
  #
  #
  # # sum_atten  = last_ray_results['sum_atten']
  # # data_loss_2 = jnp.mean((-jnp.log(jnp.exp(-jnp.abs(1-sum_atten.sum(axis=-2))))))
  #   data_loss += jnp.mean((3/(1+jnp.exp(-30*density+5))-0.02).mean(axis=-1))
  # data_loss += jnp.mean(
  #   (-jnp.log( jnp.exp(-jnp.abs(1 - weights) / 0.1) +jnp.exp(-jnp.abs(weights) / 0.05))))
  # data_loss += jnp.mean(
  #   (-jnp.log(jnp.exp(-jnp.abs(1 - weights) / 0.01) )))
  # data_loss+= jnp.mean(jnp.minimum((1-jax.lax.stop_gradient(weights_direct))**2,jax.lax.stop_gradient(weights_direct**2)+(1-weights_bs)**2))
  # data_loss+= jnp.mean(jnp.minimum((1-(weights_direct))**2,(weights_direct**2)+(1-weights_bs)**2))

  # data_loss += jnp.mean(
  #   (-jnp.log(jnp.exp(-jnp.abs(600 - weights) *0.1) +1* jnp.exp(-jnp.abs(weights) / 0.07))))
  data_loss += jnp.mean((
    (-jnp.log(6*jnp.exp(-jnp.abs(1 - weights) / 0.1) +jnp.exp(-jnp.abs(weights) / 0.1)))))
  # data_loss+= jnp.mean(-jnp.log(jnp.exp(-jnp.abs(weights.sum(axis=-1)) / 0.1))*jax.lax.stop_gradient(1-weights.sum(axis=-1)))

  data_losses = jnp.array(data_loss)

  loss = (
          sig_mult * data_losses)
  return loss

def uw_sig_med_loss(ray_history, renderings,batch, config):
  last_ray_results = renderings[-1]

  # weights = jax.lax.stop_gradient(last_ray_results['weights'])
  #weights = (last_ray_results['weights']).sum(axis=-1)
  weights = last_ray_results['sigma_bs']
  weights = weights / jnp.max(weights, axis=0, keepdims=True)
  weights_bs = last_ray_results['sigma_atten']
  weights_bs = weights_bs / jnp.max(weights_bs, axis=0, keepdims=True)
  jax.debug.print("{y}", y=jnp.max(weights_bs,axis=-1))
  jax.debug.print("{y}", y=jnp.max(weights,axis=-1))

  weights_cmed = last_ray_results['c_med']
  # sigma_bs = last_ray_results['sigma_bs']
  # sigma_atten = last_ray_results['sigma_atten']
  data_loss = 0
  # last_ray_results = renderings[-1]
  #
  # # weights = last_ray_results['joint_weights']
  ...
  # # sum_atten  = last_ray_results['sum_atten']
  # # data_loss_2 = jnp.mean((-jnp.log(jnp.exp(-jnp.abs(1-sum_atten.sum(axis=-2))))))
  data_loss += jnp.mean((jnp.std(weights_bs,axis=0))+(jnp.std(weights,axis=0)))
  # data_loss += jnp.mean(
  #   jax.nn.relu(sigma_bs-sigma_atten))
  # data_loss+= jnp.mean(jnp.minimum(((1-weights))[...,None],(weights)[...,None]+(1-weights_bs)))
...
def uw_depth_loss(ray_history, renderings,batch, config):
  last_ray_results = renderings[-1]
  gt_depth = batch.gt_depth
  errors   = batch.errors
  depth = last_ray_results['distance_mean']
  gt_depth = jax.lax.stop_gradient(jnp.where(
    gt_depth != 0, gt_depth
    , depth))
  # depth_for_loss = jax.lax.stop_gradient(jnp.abs((1-depth_mask))*depth) + depth_mask*depth
  data_loss = (((gt_depth-depth)**2)*(errors))
  final_loss = jnp.true_divide(data_loss.sum(), (data_loss!= 0).sum())
  # data_loss += jnp.mean(
  #   jax.nn.relu(sigma_bs-sigma_atten))
  # data_loss+= jnp.mean(jnp.minimum(((1-weights))[...,None],(weights)[...,None]+(1-weights_bs)))
  ...
  # data_loss += jnp.mean(
  #   (-jnp.log(jnp.exp(-jnp.abs(600 - weights) *0.1) +1* jnp.exp(-jnp.abs(weights) / 0.07))))
  data_losses = jnp.array(final_loss)

  loss = (
          config.depth_mult * data_losses)
  return loss
  ...
  def train_step(
    ...
    def loss_fn(variables):
      ...
      if config.distortion_loss_mult > 0:
        losses['distortion'] = distortion_loss(ray_history, config)  #TODO:Change Deborah
      ...
      if config.use_uw_bs_initial_loss:
        losses['uw_bs'] = uw_bs_initial_loss(rays,batch,renderings,config, bs_mult)

      if config.use_uw_sig_obj_loss:
        losses['uw_sig'] = uw_sig_obj_loss(ray_history,renderings,batch,config, sig_mult)
      if config.use_uw_sig_med_loss:
        losses['uw_med_sig'] = uw_sig_med_loss(ray_history, renderings, batch, config)
      if config.depth_loss:
        losses['depth_loss'] = uw_depth_loss(ray_history, renderings, batch, config)
